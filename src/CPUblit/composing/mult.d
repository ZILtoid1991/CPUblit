...
@nogc pure nothrow {
	...
	void mult(uint* src, uint* dest, size_t length) {
		while (length >= 4) {
			...
			__m128i src_lo = _mm_adds_epu16(_mm_unpacklo_epi8(srcV, SSE2_NULLVECT), cast(__m128i)ALPHABLEND_SSE2_CONST1);
			__m128i src_hi = _mm_adds_epu16(_mm_unpackhi_epi8(srcV, SSE2_NULLVECT), cast(__m128i)ALPHABLEND_SSE2_CONST1);
	...
	void multMV(V)(uint* src, uint* dest, size_t length, V value) {
		...
		__m128i master_256 = _mm_subs_epu16(cast(__m128i)ALPHABLEND_SSE2_CONST256, _mm_unpacklo_epi8(masterV, SSE2_NULLVECT));
		__m128i master_1 = _mm_adds_epu16(_mm_unpacklo_epi8(masterV, SSE2_NULLVECT), cast(__m128i)ALPHABLEND_SSE2_CONST1);
		while (length >= 4) {
			__m128i srcV = _mm_loadu_si128(cast(__m128i*)src);
			__m128i destV = _mm_loadu_si128(cast(__m128i*)dest);
			__m128i src_lo = _mm_adds_epu16(_mm_unpacklo_epi8(srcV, SSE2_NULLVECT), cast(__m128i)ALPHABLEND_SSE2_CONST1);
